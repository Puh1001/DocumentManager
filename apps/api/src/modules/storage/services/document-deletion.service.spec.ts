...
describe("DocumentDeletionService", () => {
  ...
  describe("checkDeletionStatus", () => {
    it("should allow deletion within 72 hours", async () => {
      ...
      (prismaService.deletionRequest.findUnique as jest.Mock).mockResolvedValue(
        null
      );
    ...
    it("should detect active deletion request", async () => {
      ...
      (prismaService.deletionRequest.findFirst as jest.Mock).mockResolvedValue(
        activeRequest
      );
  ...
  describe("submitDeletionRequest", () => {
    it("should create deletion request after expiry", async () => {
      ...
      expect(
        (prismaService as unknown as PrismaClientLike).deletionRequest.create
      ).toHaveBeenCalled();
    });
  ...
  describe("reviewRequest", () => {
    it("should approve and delete document", async () => {
      ...
      documentService.findById.mockResolvedValue(document as never);
      usersService.findById.mockResolvedValue(mockDCCUser as never);
      folderService.findById.mockResolvedValue(currentFolder as never);
      (prismaService.folder.findUnique as jest.Mock).mockResolvedValue(null);
      (prismaService.folder.create as jest.Mock).mockResolvedValue(
        deleteFolder
      );
      const departmentMock = (
        prismaService as unknown as Record<string, unknown>
      ).department as { findUnique: jest.Mock };
      departmentMock.findUnique.mockResolvedValue({
        id: "dept-1",
        name: "Test Department",
      });
      smbService.getFileStats.mockResolvedValue({
        isDirectory: () => false,
        size: 1024,
        mtime: new Date(),
        birthtime: new Date(),
