...
"""
    ...
    def test_load_env_files(self, auth_init, mock_project_root):
        ...
        # Create .env files
        cursor_env = mock_project_root / ".cursor" / ".env"
        cursor_env.parent.mkdir(parents=True, exist_ok=True)
        cursor_env.write_text("BASE_VAR=base\nOVERRIDE=cursor")

        skills_env = mock_project_root / ".cursor" / "skills" / ".env"
