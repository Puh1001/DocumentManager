---
name: skill-creator
description: Create or update Cursor skills. Use for new skills, skill references, skill scripts, optimizing existing skills, extending Cursor's capabilities.
...
**IMPORTANT:**
- Skills are not documentation, they are practical instructions for Cursor to use the tools, packages, plugins or APIs to achieve the tasks.
- Each skill teaches Cursor how to perform a specific development task, not what a tool does.
- Cursor can activate multiple skills automatically to achieve the user's request.
...
- Descriptions in metadata of `SKILL.md` files should be both concise (**less than 200 characters**) and still contains enough usecases of the references and scripts, this will help skills can be activated automatically during the implementation process of Cursor.
...
- **Referenced scripts**:
  ...
  - Make sure scripts respect `.env` file follow this order: `process.env` > `$HOME/.cursor/skills/${SKILL}/.env` (global) > `$HOME/.cursor/skills/.env` (global) > `$HOME/.cursor/.env` (global) > `./.cursor/skills/${SKILL}/.env` (cwd) > `./.cursor/skills/.env` (cwd) > `./.cursor/.env` (cwd)
...
**Why?**
Better **context engineering**: leverage **progressive disclosure** technique of Agent Skills, when agent skills are activated, Cursor will consider to load only relevant files into the context, instead of reading all long `SKILL.md` as before.
...
- **Benefits**: Keeps SKILL.md lean, loaded only when Cursor determines it's needed, makes information discoverable without hogging the context window.
...
**IMPORTANT:**
- Referenced markdown files should be also **less than 150 lines**, remember that you can always split them into multiple files (**progressive disclosure** principle).
- Referenced markdown files are practical instructions for Cursor to use the tools, packages, plugins or APIs to achieve the tasks.
- Each skill teaches Cursor how to perform a specific development task, not what a tool does.
...
When editing the (newly-generated or existing) skill, remember that the skill is being created for another instance of Cursor to use. Focus on including information that would be beneficial and non-obvious to the agent. Consider what procedural knowledge, domain-specific details, or reusable assets would help another Cursor instance execute these tasks more effectively.
