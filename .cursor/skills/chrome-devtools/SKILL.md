...
cd .cursor/skills/chrome-devtools/scripts
...
Without ImageMagick, screenshots >5MB will not be compressed (may fail to load in Gemini/Cursor).
...
All scripts are in `.cursor/skills/chrome-devtools/scripts/`
...
Screenshots are **automatically compressed** if they exceed 5MB to ensure compatibility with Gemini API and Cursor (which have 5MB limits). This uses ImageMagick internally:
...
2. Verify in `.cursor/skills/chrome-devtools/scripts/` directory
