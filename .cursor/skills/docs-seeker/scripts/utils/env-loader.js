 ...
 * Respects order: process.env > skill/.env > skills/.env > .cursor/.env
 */
 ...
 * Priority: process.env > skill/.env > skills/.env > .cursor/.env
 * @returns {Object} Merged environment variables
 */
function loadEnv() {
  const skillDir = path.resolve(__dirname, '../..');
  const skillsDir = path.resolve(skillDir, '..');
  const cursorDir = path.resolve(skillsDir, '..');

  const envPaths = [
    path.join(cursorDir, '.env'),      // Lowest priority
